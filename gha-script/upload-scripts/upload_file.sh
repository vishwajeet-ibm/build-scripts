...
if [[ $(echo "$token_request" | jq -r '.errorCode') == "null" ]]; then
    ...
    response=$(curl -X PUT -H "Authorization: bearer $token" -H "Content-Type: application/gzip" -T $1 "https://s3.us-south.cloud-object-storage.appdomain.cloud/ose-s390x-toolci-bucket-dev/$PACKAGE_NAME/$VERSION/$1")
